...
BEGIN
    ...
    ratings_stats AS (
        SELECT
            ...
            sum(COALESCE(amount_drank, 1)) as total_ratings_pints -- Calculate points from ratings
        FROM ratings
    ),
    pubs_stats AS (
        SELECT 
    ...
    pub_checkins_stats AS (
        SELECT 
            ...
            sum(COALESCE(amount_drank, 1)) as total_checkin_pints -- Calculate points from checkins
        FROM pub_checkins
    )
    SELECT json_build_object(
        ...
        'totalPintsDrank', COALESCE((SELECT total_checkin_pints FROM pub_checkins_stats), 0) + COALESCE((SELECT total_ratings_pints FROM ratings_stats), 0),
        'totalCheckinPints', (SELECT total_checkin_pints FROM pub_checkins_stats)
    ) INTO kpis_data;
